...
사용자 정보를 입력받고, 각 객체에 유일한 ID(INDEX 지정)를 부여하며, 생성된 객체 수를 추적하여 출력하는 프로그램
...
int main() {
    ...
    cout << "\n=== 매개변수 하나(fn) 전달하는 생성자 사용 ===" << endl;
    Child c2("KIM");
    c2.showFamilyName();
    c2.eat();

    cout << "\n=== 매개변수 두 개(fn, myAge) 전달하는 생성자 사용 ===" << endl;
    Child c3("PARK", 25);
    c3.showFamilyName();
    c3.eat();
